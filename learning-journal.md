...
Проблема: нельзя включить внешний сервис (email, RabbitMQ) в транзакцию бд. Поэтому если внешний сервис упадет после коммита то строка останется в базе данных а сервис к примеру не вышлет email.

Решение:
1. Сохраняй Order + OutboxMessage в одной транзакции
2. Фоновый процесс читает NotSent сообщения и отпарвляет их
3. После отправки обновлчет статус на Sent

Гарантия: если сервер упал после коммита - сообщения не потеряются, фоновый процесс найдет их при перезапуске

## Outbox Processor в production
Проблема: несколько инстансов читают одни сообщения одновременно.
Решение: SELECT FOR UPDATE SKIP LOCKED

FOR UPDATE - блокирует строку
SKIP LOCKED - другие инстансы пропускают заблокеированные строки и берут следующие свободыне
